...
void liberar_bd_time(BDTime *bd)
{
    No *atual = bd->inicio;
    while(atual){
        No *temp = atual;
...
void bd_carregar_times_csv(BDTime *bd, char *caminho_arquivo)
{
    FILE *arquivo;
...
void imprimir_classificacao(BDTime *bd, int paginacao)
{
    No *atual = bd->inicio;
    int cont = 0;
    printf("%-4d | %-14s | %-3d | %-3d | %-3d | %-4d | %-4d | %-3d | %-4d\n",
           "ID", "Time", "V", "E", "D", "GM", "GS", "S", "PG\n");
    while(atual){
        imprimir_time(atual->time); // Mostra o time
        atual = atual->proximo;
        cont++;
        if (cont % paginacao == 0){ // Controle de página
            char op;
...
void buscar_por_nome(BDTime *bd, const char *prefixo)
{
   No *atual = bd->inicio;
    int encontrou = 0;
    while (atual) {
        ...
        if (strncasecmp(t.nome, prefixo, strlen(prefixo) == 0)){
            imprimir_time(t); // Imprime se encontrou
            encontrou = 1;
        }
        atual = atual->proximo;
    }
